...
| QC-5 | No trust & safety surface | Missing requirement | Confirmed absent (Traceability §6.1); remains P1 via R-07 remainder |
...
|---|---|---|---|---|---|
| R-02 | Profile completion scattered across Profile/Dashboard | Unified activation checklist on first-run dashboard + staged nudges | Activation = strongest retention lever (K-01); underlying signals already tracked | P1 | Open — dashboard aggregation layer (`Promise.allSettled`, meta.source) makes insertion low-risk |
...
| R-07 | Completeness-only publishing | Pre-publish quality checks + report/moderation hooks into admin queue | Marketplace trust precedes scale | P1 | **Partially delivered** — `canPublishRecruiterPosting` completeness + duplicate-match gating shipped (FR-F07-1); moderation/report queue remains the open remainder (with T&S surface) |
| R-10 | Raw billing rows gate features | Entitlement service decoupling plan→permissions before live payments | Clean ADR-005 cutover; testable gating pre-revenue | P1 | Open — prerequisite for any Stripe exit; interacts with plan `features` JSON already stored |
