...
- Resolved 2026-06-27: `networkingService` now uses the generated typed Supabase client for connection, suggestion preference, profile hydration, and feed activity table paths; the mutual-count RPC remains on the compatibility client until DB function types or a backend owner API exist.
...
- Resolved 2026-06-27 at source-validation level: `infra/db/migrations/0001_initial_baseline.sql` mirrors `supabase-schema.sql`, `infra/db/generated/database.types.ts` provides source-derived database types generated from that migration with 69 public FK relationships, `apps/frontend/src/lib/supabaseClient.ts` exports a generated-`Database` typed `typedSupabase` migration boundary, and `npm run validate:schema-migrations` checks the 49-table baseline, 38 source-level RLS-enabled tables, 46 indexed tables, generated relationship metadata, generated type drift, and typed-client boundary.
