    ...
    <div className="max-w-4xl space-y-8" id="opt-view-ai">
      ...
      <form onSubmit={handleOptimize} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={`${panelClassName} space-y-3`}>
          <div className="flex items-center gap-2 text-[var(--ext-text)]">
      ...
      {(optimizing || optimized) && (
        <div className="space-y-6 rounded-lg border border-[var(--ext-border)] bg-[var(--ext-surface)] p-6 transition-all duration-300" id="ai-results-panel">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-sm font-bold text-[var(--ext-text)]">Alignment Report</span>
              {optimizing ? (
                <span className="animate-pulse text-xs text-[var(--ext-text-muted)]">Computing local keyword overlap...</span>
              ) : (
                <span className="flex items-center gap-1 text-xs font-semibold text-[var(--ext-success)]">
