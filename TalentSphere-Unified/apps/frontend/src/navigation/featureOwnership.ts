...
export const featureOwnershipRegistry: readonly FeatureOwnershipDefinition[] = [
  ...
  },
  {
    ...
    owner: { kind: 'route', routeId: 'portfolio', routePath: '/portfolio' },
    necessity: 'necessary',
    primaryPurpose: 'Own the engineering case-study showcase and category filtering for the platform.',
    userJourneyValue: 'Gives evaluators and visitors one source-verified view of shipped modules without cloning workflow surfaces.',
    mergeEvaluation: 'Keep separate from Profile and Dashboard; the showcase is read-only proof of work, not a workflow owner.',
    secondaryEntryPoints: [
      {
        surface: 'Header search',
        mode: 'search-destination',
        routeId: 'portfolio',
        routePath: '/portfolio',
        rationale: 'Route discovery can open the showcase while category filtering and case-study rendering stay owned here.',
      },
    ],
    consolidationDecision: 'Keep as a read-only showcase route; it renders static, source-verified content and owns no mutations.',
    behaviorPreservation: 'Preserve category filters, case-study grid, summary panel, and empty-filter recovery.',
  },
  {
    id: 'job-detail-view',
    label: 'Job Detail',
    owner: { kind: 'route', routeId: 'job-detail', routePath: '/jobs/:id' },
    necessity: 'necessary',
    primaryPurpose: 'Show a single job listing detail with apply flow and existing-application status.',
    userJourneyValue: 'Lets authenticated users review a job posting and apply directly without leaving the detail view.',
    mergeEvaluation: 'Keep separate as a detail view companion to the Jobs workspace; jobs listing discovers, detail owns per-role review and apply.',
    secondaryEntryPoints: [
      {
        surface: 'Jobs listing cards',
        mode: 'link',
        routeId: 'jobs',
        routePath: '/jobs',
        rationale: 'Job cards can deep-link into the detail view without duplicating apply or status behavior.',
      },
      {
        surface: 'Header search',
        mode: 'search-destination',
        routeId: 'job-detail',
        routePath: '/jobs/:id',
        rationale: 'Route discovery can surface specific job postings while apply and status tracking remain owned here.',
      },
    ],
    consolidationDecision: 'Keep as a detail-view companion to Jobs; do not merge into the jobs listing or post flow.',
    behaviorPreservation: 'Preserve job fetch, apply form, existing-application status, error and not-found recovery, and navigation.',
  },
  {
    id: 'candidate-review',
