...
const getPreviewRowLabel = (row: { label: string; value: string }) => (
  `${row.label}. ${row.value}. Owned by its domain route.`
);

const getPreviewMetricLabel = (metric: { label: string; value: string }) => `${metric.label}: ${metric.value}`;

const getPlatformPillarLabel = (feature: { title: string; description: string }) => `${feature.title}. ${feature.description}`;

const getIaPrincipleLabel = (item: string) => item;

const getPublicStatLabel = (stat: { label: string; value: string }) => `${stat.label}: ${stat.value}`;
...
const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[var(--bg-canvas)] text-[var(--text-primary)]">
      <nav aria-label="Public navigation" className="sticky top-0 z-50 border-b border-[var(--border-default)] bg-[var(--bg-panel)]/95 backdrop-blur-sm">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent text-accent-foreground">
              <Layers {...decorativeIconProps} size={18} />
            </span>
      ...
      <main aria-labelledby="landing-hero-title">
        <section aria-labelledby="landing-hero-title" className="relative overflow-hidden border-b border-[var(--border-default)] bg-[var(--bg-primary)]">
          ...
          <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(420px,1fr)] lg:px-8 lg:py-20">
            <div className="relative z-10 max-w-2xl">
              ...
              <h1 id="landing-hero-title" className="text-4xl font-semibold leading-tight text-[var(--text-primary)] sm:text-5xl lg:text-6xl">
                TalentSphere
              </h1>
              ...
              <div role="group" aria-label="Public role entry points" className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link to="/register?role=talent" className="sm:w-auto">
                  <Button size="lg" className="w-full sm:w-auto">
                    Join as Talent
                    <ArrowRight {...decorativeIconProps} className="h-4 w-4" />
                  </Button>
                </Link>
            ...
            <div role="region" aria-labelledby="landing-preview-title" className="relative z-10 rounded-lg border border-[var(--border-default)] bg-[var(--bg-panel)] p-4 shadow-lg sm:p-5">
              <div className="flex items-center justify-between border-b border-[var(--border-default)] pb-4">
                <div>
                  <p className="text-xs font-medium uppercase text-[var(--text-muted)]">Today</p>
                  <h2 id="landing-preview-title" className="mt-1 text-lg font-semibold text-[var(--text-primary)]">Career command center</h2>
                </div>
              ...
              <div role="list" aria-label="Career command center preview rows" className="mt-4 grid gap-3">
                {previewRows.map((row) => {
                  const Icon = row.icon;
                  return (
                    <div key={row.label} role="listitem" aria-label={getPreviewRowLabel(row)} className="flex items-center justify-between rounded-lg border border-[var(--border-default)] bg-[var(--bg-secondary)] p-3">
                      <span className="flex min-w-0 items-center gap-3">
                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-accent/10 text-accent">
                          <Icon {...decorativeIconProps} size={17} />
                        </span>
              ...
              <div role="list" aria-label="Workflow control principles" className="mt-4 grid gap-3 sm:grid-cols-3">
                {previewMetrics.map((item) => (
                  <div key={item.label} role="listitem" aria-label={getPreviewMetricLabel(item)} className="rounded-lg border border-[var(--border-default)] p-3">
        ...
        <section id="features" aria-labelledby="landing-features-title" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <h2 id="landing-features-title" className="sr-only">Platform pillars</h2>
          <div role="list" aria-label="Platform pillars" className="grid gap-4 md:grid-cols-3">
            {platformPillars.map((feature) => {
              const Icon = feature.icon;
              return (
                <article key={feature.title} role="listitem" aria-label={getPlatformPillarLabel(feature)} className="surface-card p-6">
                  <div className="mb-5 flex h-10 w-10 items-center justify-center rounded-md bg-accent/10 text-accent">
                    <Icon {...decorativeIconProps} size={20} />
                  </div>
        ...
        <section id="network" aria-labelledby="landing-ia-title" className="border-y border-[var(--border-default)] bg-[var(--bg-primary)]">
          <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)] lg:px-8">
            <div>
              ...
              <h2 id="landing-ia-title" className="mt-2 text-2xl font-semibold text-[var(--text-primary)]">One workflow owner for every feature.</h2>
            </div>
            <div role="list" aria-label="Feature ownership decisions" className="grid gap-3 sm:grid-cols-2">
              {iaPrinciples.map((item) => (
                <div key={item} role="listitem" aria-label={getIaPrincipleLabel(item)} className="rounded-lg border border-[var(--border-default)] bg-[var(--bg-panel)] p-4 text-sm text-[var(--text-secondary)]">
                  <Cpu {...decorativeIconProps} className="mb-3 h-4 w-4 text-accent" />
                  {item}
                </div>
              ))}
            </div>
        ...
        <section id="learning" aria-labelledby="landing-stats-title" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="surface-card p-6 sm:p-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-sm font-medium text-accent">Public platform snapshot</p>
                <h2 id="landing-stats-title" className="mt-2 text-2xl font-semibold text-[var(--text-primary)]">Current activity summary</h2>
              </div>
            ...
            <div role="list" aria-label="Public platform stats" className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
      ...
      <footer className="border-t border-[var(--border-default)] bg-[var(--bg-panel)] py-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 text-center sm:px-6 md:flex-row md:text-left lg:px-8">
          <div className="flex items-center gap-2 text-sm font-semibold">
            <Globe {...decorativeIconProps} className="h-4 w-4 text-accent" />
