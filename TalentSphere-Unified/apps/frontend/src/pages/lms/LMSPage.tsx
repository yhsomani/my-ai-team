...
import { Play, Clock, BookOpen, CheckCircle2, Search, Circle, PlayCircle } from 'lucide-react';
...
const LMSPage: React.FC = () => {
  ...
  const handleEnroll = async (courseId: string, title: string) => {
    ...
    setIsEnrolling(true);
    try {
        const enrollment = await lmsService.enrollInCourse(courseId, user.id);
        setEnrollments(prev => {
          const withoutExisting = prev.filter(item => item.courseId !== courseId);
  ...
  const handleMarkLessonComplete = async () => {
    ...
    try {
      ...
      addToast({ type: 'error', title: 'Progress not saved', message: 'Please try again later.' });
    } finally {
                  ...
                  return (
                    <Card key={course.id} className="p-4">
                      <div className="flex h-full flex-col justify-between gap-4">
                        ...
                        <Button size="sm" className="w-full" onClick={() => handleCourseClick(course)}>
                ...
                {recommendedCourses.map((course) => (
                  <Card key={course.id} className="p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <Button size="sm" variant="outline" className="w-full sm:w-auto" onClick={() => handleCourseClick(course)}>
                        Start
                      </Button>
                    </div>
                  </Card>
