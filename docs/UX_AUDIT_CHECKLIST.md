...
- Compact status badges must keep visible text, use optional hidden descriptions for abbreviated labels, wrap within their parent surface, and avoid implicit live-region behavior unless the owning screen intentionally supplies it.

## Current Validation Evidence

The latest local UI/UX validation snapshot is 2026-06-30:

- `npm run test:ia --workspace talentsphere-web` passed with 2 files / 19 tests covering route ownership, role alignment, utility route placement, and route-purpose helper matching.
- `npm run validate:ui-design-system`, `npm run validate:docs-lifecycle`, and `npm run validate:module-manifest` passed.
- `npm run lint --workspace talentsphere-web` and `npm run build --workspace talentsphere-web` passed.
- `npm run test:unit --workspace talentsphere-web` passed with 112 test files and 626 tests.
- `npm run test:a11y --workspace talentsphere-web` passed with 41 Chromium route semantics tests; the current audit also enforces route-specific main landmark names for authenticated shell screens.
- `npm run test:unit --workspace talentsphere-web -- --run src/navigation/routeRegistry.test.ts src/components/shared/ResponsiveLayout.test.tsx` passed with 15 tests covering route matching, route-derived shell landmark names, command-route naming, dynamic profile-detail naming, unknown-route fallback, skip target metadata, sidebar offset behavior, logout wiring, and unauthenticated passthrough.
- `npm run test:e2e --workspace talentsphere-web -- tests/keyboard-navigation.spec.ts --project=chromium --reporter=line` passed with 14 Chromium keyboard tests after the route-purpose landmark update.
- `npm run test:e2e --workspace talentsphere-web -- tests/visual-layout.spec.ts --project=chromium --reporter=line` passed with 41 Chromium route layout tests after the route audit fixture update.
- `npm run test:e2e:chromium --workspace talentsphere-web` passed with 235 Chromium tests covering public routes, authenticated dashboards, domain workflows, accessibility semantics, rendered route contrast, keyboard navigation, route access, notifications, visual layout, and performance smoke checks.
- `npm run test:contrast:all --workspace talentsphere-web` passed with 123 rendered route contrast tests across Chromium, Firefox, and WebKit.
- `npm run test:extension-popup-ux`, `npm run test:extension-options-ux`, `cd chrome-extension-project && npm run build`, and `npm run test:extension-runtime-smoke` passed for the companion extension popup/options UI, rendered extension contrast, and live MV3 runtime smoke.
...
- Every major route or surface must record a user-journey value so screen necessity is tied to user outcomes, not only implementation history.
- Every major route or surface must record a merge evaluation so consolidation remains a reviewed decision rather than an implicit route removal.
...
- Dashboard-hosted domain appearances are limited to the approved summary or explicit link handoffs in `featureOwnership.ts`; adding another domain to Dashboard requires updating the IA contract and tests.
- Primary and account navigation must stay limited to necessary source-backed route owners; utility routes such as Billing, Resume, Career Path, and Post Job remain command-search destinations unless a reviewed IA change promotes them.
- Searchable utility routes must be documented as `search-destination` secondary entries in `featureOwnership.ts`.
...
- `npm run test:ia` validates route ownership, public route ownership, user-journey value coverage, merge-evaluation coverage, secondary entry-point references, dashboard placement modes, primary/account navigation ownership, utility search-destination documentation, role alignment, and duplicate-owner prevention.
...
- Public navigation, role-entry CTAs, career preview rows, workflow control principles, platform pillars, feature-ownership decisions, and public stats now expose named navigation/group/region/list/listitem semantics.
...
- `apps/frontend/tests/auth.spec.ts` verifies public landing role entry-point destinations, named landing structure, public stats visibility, and no horizontal document overflow at a 390px mobile viewport.
...
- Auth entry screens expose an H1-named public auth main landmark, named authentication panel, named alternate-entry navigation, named Login/Register forms, Account Type group semantics, and a named registration next-step status.
...
- `apps/frontend/tests/auth-entry-workflow.spec.ts` verifies configured email login, named auth-entry structure, accessible invalid-credential errors, inactive provider-control removal, and registration role-intent selection across Chromium, Firefox, and WebKit.
- `apps/frontend/src/pages/auth/AuthEntry.test.tsx` verifies named Login/Register shell/form/account-type/next-step semantics, safe provider-failure copy, raw provider-error exclusion, invalid-credential copy, weak-password guidance, and registration role-intent context.
...
- Cross-domain Dashboard placements must remain one of the source-approved summary or explicit handoff link entries.
...
- Talent dashboard presentation is summary-only: activation checklist, freshness status, applications/messages/XP/level metrics, recent opportunities, quick actions, and active challenge summaries. Metrics, checklist tasks, quick actions, recent opportunities, and active challenge summaries expose named list semantics.
- Recruiter dashboard presentation is summary-only: recruiter setup checklist, freshness status, jobs/applicant/new/offer metrics, recent applications, and quick actions. Metrics, setup tasks, quick actions, and recent applications expose named list semantics.
...
- `apps/frontend/src/pages/dashboard/DashboardPage.test.tsx` verifies safe Dashboard status-strip issue copy, dashboard summary list semantics, raw provider-error exclusion, and retry through the existing refresh workflow.
- Unreferenced legacy dashboard widget components under `pages/dashboard/components` were removed after source search proved they were not imported by the active dashboard route; dashboard summaries now stay in `DashboardPage` local presentation helpers instead of a second unused widget path.
...
- Admin summary metrics, product analytics insights, scheduled automation status, service health, and audit log surfaces expose named regions, lists, tables, and row labels so operational data can be traversed by purpose without duplicating user or recruiter dashboard content.
...
- `apps/frontend/src/pages/admin/AdminDashboard.test.tsx` verifies safe Admin Console failed-load copy, raw error-message exclusion, retry through the existing refresh workflow, semantic structure for Admin metrics, analytics insights, scheduled automations, service health, and audit rows, and decorative Admin icon treatment.
...
- The Jobs route now presents tabs, a named route search surface with active-tab helper text, a named Explore filter group with filter helper text, saved searches, hidden preference controls, and status summaries as one framed workspace tool instead of disconnected blocks.
...
- Job, application, and recruiter posting cards now share stable dimensions and token-backed card styling, and Explore results, Applied applications, My Posts recruiter postings, and saved searches expose named list/listitem semantics so result grids scan consistently across tabs.
- Applied application Details timelines now expose a named status timeline region plus recorded-event, rejected-fallback, and inferred-step list/listitem semantics so application status history is discoverable without duplicating application management outside Jobs.
...
- `apps/frontend/src/pages/jobs/JobsPage.test.tsx` verifies safe recovery paths plus semantic structure for the Jobs search surface, Explore filter group, Explore results, Applied applications, My Posts recruiter postings, saved searches, and Applied application status timelines.
...
- The Post Job route now uses token-backed form controls, framed template/history/company panels, compact review-state summaries, warning duplicate panels, stable mobile footer actions, and named form/region/group/list semantics for template controls, draft history, company context, editable draft fields, review metadata, requirement preview, duplicate/change review lists, and final actions.
...
- `apps/frontend/src/pages/jobs/PostJobPage.test.tsx` verifies safe edit-draft context failed-load copy, safe company context failed-load copy, safe company and draft action-failure copy, accurate template save/delete and draft-history browser-storage failure copy, raw provider/quota-error exclusion, Post Job section semantics, and retry or preserved state through the existing recruiter jobs, company lookup, company action, draft save, template, and draft-history workflows.
...
- The Candidates route now presents a named search surface plus a named focus/sort/page-size/pagination review-control group as one framed workspace tool; review analytics, bulk actions, candidate rows, detail sections, and confirmation modals use token-backed surfaces and status colors.
- Candidate search and review controls expose hidden helper text so assistive technologies can distinguish candidate-page search from display-only review queue controls.
- Candidate review metrics and application rows expose named list semantics with descriptive scorecard coverage, evidence gap, sync, candidate, job, status, and email labels.
...
- Candidate Details sections expose named region/list semantics for queue navigation, identity summary, application metadata, advisory signal, submitted materials, review guidance, interview-plan drafts, scorecard dimensions, and recruiter notes without moving actions or changing persistence.
...
- `apps/frontend/src/pages/candidates/CandidatesPage.test.tsx` verifies safe Candidates application-list failed-load copy, single-status failure copy, all-failed bulk status copy, raw provider-error exclusion, candidate search/control semantics, candidate review metric/application-row list semantics, Candidate Details section semantics, and retry through the existing candidate application page load and confirmation workflows.
...
- Profile summary, skill chips, summary metrics, completion checklist, local suggestions, tabs, experience rows, education rows, and achievement cards now expose named region/list/listitem/progressbar/tabpanel semantics without moving actions or changing persistence.
...
- `apps/frontend/src/pages/profile/ProfilePage.test.tsx` verifies safe Profile failed-load copy, safe basic-save/completion-row/row-delete/avatar action-failure copy, raw provider-error exclusion, Profile section/list/tabpanel semantics, and retry through the existing profile load and action workflows.
...
- Resume Builder now exposes a named Resume actions toolbar, Resume sections tablist/tabpanels, editor-field lists, import-review field/skill/profile-row lists, export activity artifact/history lists, and preview regions while preserving existing commands and profile-service ownership.
- Resume Builder exported HTML now follows the shared typography and color-token rules by using normal section-heading letter spacing plus explicit standalone export tokens; `validate:ui-design-system` rejects future nonzero literal CSS `letter-spacing` declarations and ad hoc hard-coded hex or raw functional color literals in scanned UI source.
...
- `apps/frontend/src/pages/profile/ResumeBuilder.test.tsx` verifies safe Resume Builder profile-data failed-load copy, save/upload/delete/import-save action-failure copy, raw provider-error exclusion, named editor/import/export/preview semantics, and retry through the existing resume load and action workflows.
...
- Settings section navigation exposes a vertical tablist with controlled tabpanels and arrow/Home/End keyboard movement; notification channels, activity alerts, delivery controls, security actions, and billing summary metrics expose named list/listitem or region semantics.
...
- `apps/frontend/src/pages/settings/SettingsPage.test.tsx` verifies safe notification/billing failed-load copy, safe profile-save/password-reset/account-deactivation failure copy, raw provider-error exclusion, Settings tab/list semantics, and retry through the existing settings load and action workflows.
...
- The Learning route now presents tabs, a named catalog search surface, result counts, page-size selection, and pagination in a named catalog controls group as one framed catalog workspace instead of disconnected controls.
- Learning catalog search and pagination controls expose hidden helper text so assistive technologies can distinguish course search from display-only catalog view controls.
- Continue Learning, Recommended Next, catalog cards, loading skeletons, progress warnings, and course-detail modal sections use token-backed surfaces, stable card dimensions, shared progress-track styling, and contextual progressbar semantics for visible course progress.
...
- `apps/frontend/src/pages/lms/LMSPage.test.tsx` verifies the named Learning catalog search surface, catalog search helper relationship, named Learning catalog controls group, catalog-control helper relationship, and Continue Learning, catalog-card, and course-detail progress tracks with `progressbar` names, min/max/current values, and readable percent text.
- No LMS route, Redux query state, cursor behavior, tab behavior, search behavior, enrollment behavior, lesson-completion behavior, progress calculation behavior, AI suggestion review behavior, toast behavior, or analytics call was removed in this Learning UI pass. LMS progress persistence now treats Supabase fallback read/write errors as failed saves instead of presenting unsaved lesson progress as complete.
...
- The Challenges route now presents category filters as one framed assessment workspace control with a named `Challenge category filters` group, helper text, pressed state, and stable audit marker; challenge cards, loading skeletons, prompt/editor panels, sample cases, local-check results, latest submission, retry history, and reset review use token-backed surfaces, stable dimensions, and semantic list/region labels.
...
- `apps/frontend/src/pages/challenges/ChallengesPage.test.tsx` verifies safe catalog failed-load copy, safe submission action-failure copy, challenge category-filter semantics, challenge catalog/workspace semantics, contextual editor description, sample-case labels, retry-history attempt labels, raw provider-error exclusion, unchanged retry history after failed persistence, and retry through the existing fetch and Submit Solution workflows.
...
- The Networking route now presents tabs, counts, active-view search, hidden-suggestion restore, and reminder status as one named framed workspace control; suggestion, incoming request, sent request, and accepted-connection card groups expose list/listitem semantics, person-specific card labels, person-specific repeated action labels, decorative search/card/action/status/metadata/preview icons, and decorative avatar initials while using token-backed surfaces, stable dimensions, truncation, and shared empty/loading/error states.
- Network view controls and active-view search expose hidden helper text so assistive technologies can distinguish relationship-view navigation from active-view person search.
...
- Unreferenced legacy networking subcomponents under `pages/networking/components` were removed after source search confirmed the active `NetworkingPage` route, route modules, tests, and docs do not import them.
- Browser-level Networking coverage in `tests/networking-workflow.spec.ts` verifies deterministic suggestion rendering, profile preview, hidden-suggestion hide/restore preference sync, reviewed connection request payloads, incoming accept/decline payloads, sent reminder set/clear status, withdraw payloads, accepted connection profile preview, keyboard preview activation, and full-profile popup route targets across Chromium, Firefox, and WebKit with deterministic local data boundaries; the focused Chromium workflow also passed after the person-specific action labels were added.
- `apps/frontend/src/pages/networking/NetworkingPage.test.tsx` verifies safe Networking failed-load and action-failure copy, Network view/search control semantics, list/listitem card semantics, person-specific action labels, decorative search/card/preview icons, decorative avatar initials, raw error-message exclusion, and retry through the existing fetch, Connect, Accept, Decline, and Withdraw workflows.
...
- The Messaging route now presents the conversation list and active thread as one framed workspace with token-backed list rows, descriptive conversation-row accessible names that include presence status, active-row current state, decorative search/retry icons, decorative fallback initials and online dots, decorative thread avatar initials and realtime status dots, decorative thread/composer icons, a named text composer form with review-before-send helper text, bounded message bubbles, attachment and suggested-reply composer panels, visible realtime subscription state, stable mobile panel switching, and normalized loading, empty, error, and retry states. Opening the attachment panel moves focus to the link field, and the hidden file input stays behind the visible Upload file button instead of becoming an extra tab stop.
...
- Message send, attachment upload, and visible mark-read failures now use fixed safe copy, hide raw provider errors, keep failed messages/unread state available, and preserve retry/status recovery through explicitly named Messaging action workflows.
- Unreferenced legacy messaging subcomponents under `pages/messaging/components` were removed after source search confirmed the active `MessagingPage` route, route modules, tests, and docs do not import them.
- Browser-level Messaging coverage in `tests/messaging-workflow.spec.ts` verifies deterministic conversation rendering, active-thread selection, message-history rendering, text-send payloads, failed-send retry, keyboard attachment-link focus order, uploaded-file and linked attachment send payloads, keyboard visible mark-read update payload/feedback, keyboard older-history loading, sent feedback, and persisted sent-message/attachment display across Chromium, Firefox, and WebKit with deterministic local data boundaries; the focused Chromium workflow also passed after the descriptive Retry failed message and mark-read labels were added.
- `apps/frontend/src/pages/messaging/MessagingPage.test.tsx` verifies safe conversation/message-history failed-load copy, descriptive conversation-row labels with presence status, named composer form semantics, review-before-send helper wiring, decorative conversation-list icons/initials/status dots, decorative thread/composer icons, decorative thread avatar initials and realtime status dots, thread-action labels, send/upload/mark-read action-failure copy, raw provider-error exclusion, failed-message retry, and unread-state preservation through the existing Messaging workflows.
...
- The Billing route now presents plan comparison, payment method, and transaction history with token-backed sections, mobile-safe card grids, accessible plan-list semantics, named payment-method and transaction-history regions, transaction list labels, plan-specific comparison labels, feature-list labels, plan-specific Current/Review action labels, wrapped user/provider text, status badges, inline demo/degraded state, and stable modal action layouts.
...
- `apps/frontend/src/pages/billing/BillingPage.test.tsx` verifies safe provider-unavailable and action-failure copy, accessible plan-list semantics, plan-specific comparison/action labels, feature-list labels, payment-method/transaction-history semantics, raw provider-error exclusion, and retry through the existing billing data load, Review Plan, and Update Payment Method workflows.
...
- Resume actions, import review, editor fields, export activity, artifact lists, export history, and preview sections now have named toolbar, region, list/listitem, and tabpanel semantics for easier keyboard and assistive-technology traversal.
...
- Popup storage failures now show one safe live warning for tracked-job, scanned-draft, or local-diagnostics load/save failures, and the footer changes to a degraded local-storage label. The warning does not expose quota exceptions, Chrome runtime errors, storage keys, or token-like values, removed storage keys reset to the hook initial value instead of crashing mounted views, and successful persistence behavior is unchanged.
- Built-popup runtime smoke now verifies install-time storage schema marker creation, popup default rendered contrast, popup storage load and quota-pressure save warning rendering, warning rendered contrast, safe visible copy, and degraded footer state in a live Chromium-compatible MV3 runtime.
...
- Options storage failures now show safe live warnings for preparation cards and local settings when a load/save operation fails. The warnings do not expose quota exceptions, Chrome runtime errors, or storage keys, removed storage keys reset to the hook initial value instead of crashing mounted views, and successful persistence behavior is unchanged.
- Built-options runtime smoke now verifies Resume Match empty-field validation, completed local report semantics, Interview Planner empty-topic validation, prep-card creation, prep-card pressed-state toggling, Prep Clear All cancel/confirm, Settings reset cancel/confirm, prep/settings storage load warnings, prep/settings quota-pressure save warnings, rendered contrast for Resume Match, Interview Planner, Settings, and options storage-warning states, and `ts_prep` persistence through real `chrome.storage.local` in a live Chromium-compatible MV3 runtime; published/prior-version update migration remains a separate release gap.
- Options shell, Resume Match panels/results, Interview Planner form/cards/clear review, Settings local-only sync review, toggles, prep reset review, and options HTML wrapper now use extension-local tokens, responsive layout, visible focus rings, 8px radius, and wrapped long text; popup/options HTML wrappers no longer carry hard-coded fallback colors outside extension token definitions.
...
- The AI route now uses token-backed review queue, clear-chat review, chat message, prompt draft, and composer surfaces with bounded mobile message bubbles and product-language status copy.
- Unreferenced legacy AI subcomponents under `apps/frontend/src/pages/ai/components` were removed after source search confirmed the active `AIAssistant` route, route modules, tests, and docs do not import them.
- AI Review Queue draft recommendations, the AI conversation, and prompt suggestions now expose named list/log/article semantics so users can traverse draft recommendations, chat messages, and empty-state prompts by purpose.
- AI Assistant badge, review-queue, recommendation-card, conversation-avatar, draft-prompt, and composer icons are decorative; action meaning remains in visible text, message article labels, explicit accessible labels, and the named prompt-composer form with review-first input helper text.
...
- Career Path generated guidance now exposes a named workspace, generated-path region, required-skill list, milestone list with completed/pending labels, and review-boundary region/list semantics.
- Career Path unavailable-state, retry, AI handoff, generated-path, milestone-state, Learning handoff, and review-boundary icons are decorative; generated guidance meaning remains in headings, region/list labels, listitem labels, and visible button text.
...
- Browser-level Career Path coverage in `tests/career-path-workflow.spec.ts` verifies generated guidance rendering, generated-guidance semantics, review boundaries, Learning handoff, AI Assistant handoff, malformed generated data as retryable, retry success, and provider-unavailable state across Chromium, Firefox, and WebKit with deterministic local data boundaries.
- `apps/frontend/src/pages/ai/AICareerPath.test.tsx` verifies generated guidance semantics, safe provider-unavailable copy, raw AI/provider-error exclusion, retry through the existing career-path generation workflow, and decorative unavailable/guidance/handoff/review-boundary icon treatment.
...
- Shared `Skeleton` placeholders expose shared skeleton root/compatibility markers, are decorative by default, preserve explicit loading status semantics when supplied, use reduced-motion-aware pulse animation, and are source-guarded so static skeleton usage reserves layout space.
- Empty states explain what is absent and provide one next action when useful.
- Shared `EmptyState` surfaces expose shared empty-state root/icon/title/description/action markers with matching compatibility `data-slot` markers, require connected description text, expose named regions, hide decorative icons, use non-submit recovery buttons, and preserve caller-owned recovery actions.
- Shared `Button` controls expose shared button and compatibility slot markers, preserve caller-owned button types, keep loading labels available, hide loading icons from assistive technologies, and keep long labels and icon buttons dimensionally safe.
- Shared `Badge` surfaces expose shared badge root/compatibility markers, keep status meaning in visible text, connect optional compact-label descriptions without replacing caller descriptions, and do not create implicit live regions.
- Shared `Toggle` switches expose shared toggle root/switch markers and compatibility slots, bind visible label and description text to the control, use non-submit switch buttons inside forms, and require an explicit accessible name when no visible label is present.
...
- Icon-only actions have accessible labels or visible text, and route-level icon-only controls keep at least 32px visible targets plus explicit focus-visible styling.
- Shared page headers must expose shared header/copy/title/action `data-ui` markers with matching compatibility `data-slot` markers, expose one H1 title, connect supporting description copy, keep status badges adjacent to the title, and group page-level actions without moving route-owned controls.
- Shared action buttons must expose the shared button marker, keep loading labels visible, hide loading icons from assistive technologies, prevent duplicate loading submissions, preserve caller-owned submit/button types, allow long text labels to wrap within their parent surface, and preserve fixed dimensions of at least 32px plus focus-visible rings for icon-only controls.
- Shared cards must expose shared card/subpart markers, preserve caller-owned semantics, constrain their own width in grids, wrap long titles/descriptions, allow footer actions to wrap instead of forcing horizontal overflow, and keep legacy `AuraCard` imports wired to the same shared primitives.
- Shared primitive reuse should import primitives, typography helpers, and compatibility `PageTemplate` from `components/shared` for new UI, and the shared barrel must stay an export surface only: no route logic, domain workflows, service calls, analytics, or alternate primitive implementations.
- Shared images must expose shared image/media/loading/fallback markers with matching compatibility `data-slot` markers, preserve caller-owned alt text and lifecycle handlers, use lazy/async defaults unless callers override them, keep delayed loading skeletons decorative, expose named unavailable fallbacks for meaningful images, and hide decorative failed-image fallbacks.
- Shared tabs must expose shared tablist/trigger/icon markers with matching compatibility `data-slot` markers, expose selected-state metadata, use a named horizontal tablist, keep icons decorative, connect tabs to panels when stable IDs are supplied, and keep selected tab state aligned with DOM focus after Arrow/Home/End keyboard movement.
- Exported legacy post, stat, and status helpers must expose named article, metric group, and status semantics while hiding decorative initials and icons from assistive technologies.
- Exported typography and page-template helpers must preserve native text semantics, caller attributes, wrapping-safe defaults, auditable page-template shell/content markers, and named main content landmarks without moving route-owned controls.
- Authenticated shell layout must expose a keyboard-visible skip link to one named application content `main` landmark, expose shell root/skip-link/main/page `data-ui` markers with matching compatibility `data-slot` markers, keep sidebar/header route-entry ownership unchanged, preserve userless passthrough, and maintain responsive sidebar offsets.
- Exported legacy navbar helpers must expose named navigation landmarks, active-route `aria-current` state, explicit mobile-menu expanded/controlled relationships, decorative icon treatment, and unchanged auth/logout routing.
- Shared theme provider behavior must preserve the `aura-theme` key, root `light`/`dark` classes, stored-theme priority, system preference fallback, and current-session theme toggling even when browser storage is unavailable.
- Exported legacy mobile menu helpers must expose distinct shortcut and expanded navigation landmarks, keep shortcut placement route-only, mark active routes, hide decorative icons, and preserve close/theme/logout callbacks.
- Active sidebar shells must keep collapsed brand/home links named, primary and expanded mobile navigation landmarks distinct, active routes marked, decorative shell icons/dividers hidden, and close/theme/sign-out/collapse callbacks unchanged.
- Active header shells must keep mobile-navigation and notification trigger controls named, controlled relationships explicit, visual shell icons/unread dots/avatar initials hidden, and command-search plus notification workflows unchanged.
- Command search must keep route meaning in the search landmark, combobox, listbox, option names, and no-result status while hiding visual search/result/arrow icons unless those icons gain distinct user-facing meaning.
- Shared input fields must expose input-field/input root markers plus label/control/icon/helper/error subpart markers with matching compatibility `data-slot` markers, connect visible labels, helper text, and error text programmatically, hide decorative leading icons from assistive technologies, and keep validation copy plus submission behavior route-owned.
- Legacy stat/post helper cards must expose stat-card and post-card root/subpart `data-ui` markers with matching compatibility `data-slot` markers, preserve named metric/article semantics, and keep avatar/stat icons decorative.
- Shared status strips must expose status-bar root/subpart `data-ui` markers with matching compatibility `data-slot` markers, keep icons and separators decorative, and remain summary-only surfaces rather than workflow action owners.
...
- Each audited route must render its accessible page heading, avoid visible `undefined` placeholder text, produce no browser page exceptions, avoid horizontal document overflow, avoid clipped visible text inside buttons, links, tabs, or other route controls, keep named icon-only controls at least 32px wide and tall, and keep named icon-only controls focus-visible by style contract.
...
- `apps/frontend/tests/color-contrast.spec.ts` is the broad browser guardrail for rendered major-route text contrast.
- The audit reuses deterministic route fixtures and covers desktop/mobile public, talent, recruiter, and admin route states, including public Not Found recovery.
- It computes effective rendered text/background contrast for visible direct text, uses 4.5:1 for normal text and 3:1 for large text, and skips hidden or disabled content.
- `npm run test:contrast` runs the audit in Chromium for a focused local gate; `npm run test:contrast:all` runs it across Chromium, Firefox, and WebKit for all-browser route contrast evidence.
- Passing this web route audit does not replace full WCAG review, chart/canvas/image contrast review, extension all-browser/deployed contrast checks, deployed CDN contrast checks, or manual visual QA. Extension popup/options runtime contrast is covered separately by `npm run test:extension-runtime-smoke`.
...
- The route semantics audit includes the public Not Found recovery route so invalid-route recovery keeps one main landmark, one visible `h1`, and named recovery actions.
...
- Each audited screen must expose exactly one visible main landmark with the expected route-purpose name, exactly one visible `h1`, named interactive controls, programmatically labeled form controls, unique names for multiple visible navigation landmarks, readable visible status/alert feedback regions, and alt/decorative treatment for visible images.
- Authenticated shell main landmark names are derived from the `getApplicationContentLabel` helper in `routeRegistry.ts`, so protected screens expose names such as `Dashboard application content`, `Profile Detail application content`, and `Post Job application content` without adding a second workflow owner or changing route behavior.
...
- `ProtectedRoute` loading must remain a polite busy status with stable `data-ui` markers and decorative spinner treatment, while Login redirects, Dashboard recovery for forbidden roles, and allowed-route rendering stay owned by the route registry and auth state.
- Passing this audit does not replace a full WCAG audit, screen-reader testing, keyboard walkthroughs, or rendered page-level color-contrast tooling. It prevents common semantic regressions while the redesign continues; source-level core token contrast is guarded separately by `npm run validate:ui-design-system`.

## Shared Feedback Guardrail

- Shared toast notifications are transient feedback only; persistent failed, degraded, fallback, or retryable states must remain visible inline in the owning screen.
- The shared `ToastProvider` exposes shared toast stack/item/subpart markers with matching compatibility `data-slot` markers, a named Toast notifications region, toast-type-specific status/alert labels and metadata, assertive atomic error alerts, decorative icons hidden from assistive technologies, non-submit notification-specific dismiss controls, and unchanged auto-dismiss timing.
- `apps/frontend/src/components/shared/Toast.test.tsx` verifies the shared toast accessibility contract so route workflows can reuse toast feedback without duplicating notification semantics.
- The older `context/ToastContext` remains only as a compatibility provider for realtime notification imports. It must expose a distinct Realtime toast notifications region, keyboard-reachable Dismiss controls, preserved click-to-dismiss behavior, and guarded 5-second auto-dismiss timing until the path is retired or migrated.
- `apps/frontend/src/context/ToastContext.test.tsx` verifies the compatibility toast contract.
...
- The audit verifies authenticated shell skip-to-content focus, command search focus, result arrow selection, submit behavior, notification popover reminder activation, account notification Mark read, account notification destination activation, notification Load more pagination, Escape focus restoration, mobile bottom-navigation keyboard activation, mobile slide-over named navigation/current-route activation, collapsed desktop sidebar route/footer control names, shared tab Arrow/Home/End roving focus, shared modal focus trap/restore behavior, and login form Enter submission.
- Shell navigation surfaces must stay route-entry surfaces only: the desktop sidebar, mobile slide-over, legacy mobile menu, and bottom mobile navigation use route-registry role filtering, active `aria-current` state, and distinct landmark names without duplicating domain workflow controls.
- `apps/frontend/tests/command-search-workflow.spec.ts` verifies `CommandSearch` label-ranked route discovery, named search/listbox/no-result semantics, role-filtered utility destinations, no-result states, and recruiter command-route navigation across Chromium, Firefox, and WebKit with deterministic local data boundaries.
- `apps/frontend/src/components/layout/CommandSearch.test.tsx` verifies focused `CommandSearch` unit semantics: named/described combobox and listbox relationships, decorative search/result/arrow icons, label-ranked click navigation, shell close callbacks, and role-filtered no-result behavior without navigation.
- `CommandSearch` must stay a secondary route-discovery surface: route labels should outrank keyword-only matches, forbidden-role routes must not appear, result options should include route label plus route-registry description, no-result states must be named and visible, and utility route discovery must be documented in `featureOwnership.ts` rather than becoming a second workflow owner.
- Shared `Tabs` must keep keyboard-selected tab state, selected-state metadata, and DOM focus aligned so the active tab remains the next tab stop after arrow navigation.
...
- Shared `AuraModal` must expose shared modal structure `data-ui` markers with matching compatibility `data-slot` markers, hide backdrop and close icons from assistive technologies, keep the close control non-submit, keep focus inside open dialogs, lock background page scrolling while open, support Escape close, and restore focus to the opener when the dialog closes.
- `apps/frontend/src/components/shared/AuraModal.test.tsx` verifies the shared modal structure and scroll containment contract, including prior body overflow restoration after close.
...
- `npm run test:unit`
- `npm run test:ia`
- `npm run validate:ui-design-system` for legacy visual classes, undefined tokens, hard-coded color literals, and core token contrast
- `npm run test:a11y`
