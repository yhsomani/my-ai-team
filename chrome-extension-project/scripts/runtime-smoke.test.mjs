...
// Branded Google Chrome refuses unpacked extensions from the command line
const UNPACKED_LOAD_REFUSAL = '--load-extension is not allowed in Google Chrome';
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `
        (async () => {
          ...
          await waitFor(
            () => {
              const button = document.querySelector('#start-optimization-btn');
              if (!button || button.disabled) return false;
              const style = getComputedStyle(button);
              return style.backgroundColor === 'rgb(15, 118, 110)'
                && style.color === 'rgb(255, 255, 255)';
            },
            'Resume Match button color transition settle'
          );
...
const runSmokeWithBrowser = async (browserPath) => {
  ...
  try {
    ...
    if (stderr.includes(UNPACKED_LOAD_REFUSAL)) {
      throw new Error(UNPACKED_LOAD_REFUSAL);
    }
...
for (const browserPath of candidateBrowserPaths()) {
  try {
    ...
    if (!forcedBrowser && message.includes(UNPACKED_LOAD_REFUSAL)) {
      console.warn(`skipping ${browserPath}: branded Google Chrome does not load unpacked extensions (coverage falls to Edge/Chromium)`);
      continue;
    }
